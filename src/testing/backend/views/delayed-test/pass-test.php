 ...
 * @var \backend\models\PassTestForm $form
 */
...
<div class="row">
        ...
        <div class="box box-info">
            ...
            <div class="box-body">
                <dl>
                    ...
                    <dd><?= $form->delayed_test->test->name ?></dd>
                    <dt>Автор теста</dt>
                    <dd><?= $form->delayed_test->test->author->username ?></dd>
                    <dt>Время создания</dt>
                    <dd><?= date("d-m-Y H:i", $form->delayed_test->test->created_at + 3 * 3600) ?></dd>
                    <dt>Тест выполнял</dt>
                    <dd><?= $form->delayed_test->user->username ?></dd>
                </dl>
            </div>
    ...
    <div class="col-md-6 col-xs-9">
        <form id="pass-test-form" method="post" action="/delayed-test/pass-test">
            <?= yii\helpers\Html:: hiddenInput(\Yii:: $app->getRequest()->csrfParam, \Yii:: $app->getRequest()->getCsrfToken(), []) ?>
            <input id="pass-test-id" type="hidden" value="<?= $form->delayed_test->test->id ?>"
                   name="PassTestForm[test][id]">
            <input id="delayed-test-id" type="hidden" value="<?= $form->delayed_test->id ?>"
                   name="PassTestForm[test][delayed_test_id]">
            <div id="pass-test-box" style="display: block">
                <div class="box box-success">
                    ...
                    <div id="pass-test-questions" class="box-body">
                        <?php foreach ($form->delayed_test->test->questions as $question) { ?>
                            <div id="question-number-<?= $question_count ?>"
                                 style="display:<?= ($question_count == 0) ? 'block' : 'none' ?>">
                                <input type="hidden" name="PassTestForm[test][questions][<?= $question->id ?>][text]" value="<?= $question->text ?>">
                                <h4><?= $question->text ?></h4>
